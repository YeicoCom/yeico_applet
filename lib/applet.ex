defmodule Applet do
  ...
  def reset!(except \\ nil) do
    ...
    list
    |> Enum.filter(fn route -> route != except end)
    |> Enum.each(fn route -> :ok = Store.delete(route) end)

    list = started()

    list
    |> Enum.filter(fn {_, route} -> route != except end)
    |> Enum.each(fn {_, route} -> stop!(route) end)
  end
