defmodule AppletServerTest do
  ...
  test "applet server" do
    ...
    assert {:ok, "ok save /tmp/yeico_applet_server_test.exs\n"} = Tcp.read(client)
    :ok = Tcp.write(client, "list saved\n")
    ...
    assert {:ok, "ok start yeico_applet_server_test\n"} = Tcp.read(client)
    ...
    assert {:ok, "ok stop yeico_applet_server_test\n"} = Tcp.read(client)
    :ok = Tcp.write(client, "list started\n")
    assert {:ok, "ok list started\n"} = Tcp.read(client)
    :ok = Tcp.write(client, "delete yeico_applet_server_test\n")
    assert {:ok, "ok delete yeico_applet_server_test\n"} = Tcp.read(client)
    :ok = Tcp.write(client, "list saved\n")
    assert {:ok, "ok list saved\n"} = Tcp.read(client)
    :ok = Tcp.close(client)
  end
